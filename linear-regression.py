...
# Program: Linear Regression from scratch, no imports.
...
class LinearRegression:
    ...
    def rss(self):
        if self.is_trained:
            resid = [(self.Y[i] - dot_prod(self.b, [1] + self.X[i], p + 1))**2 for i in range(n)]
    ...
    def fit(self, Y, X, display_error=False):
        ...
        for _ in range(self.max_iter):
            rss_aux = [-(self.Y[i] - dot_prod(self.b, [1] + self.X[i], p + 1)) /
                       n for i in range(n)]
